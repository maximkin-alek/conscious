...
function generateReport(analysisResults) {
  ...
  const totalIssues = analysisResults.reduce(
    (sum, r) => sum + r.stats.issuesCount,
    0,
  );
  const totalWarnings = analysisResults.reduce(
    (sum, r) => sum + r.stats.warningsCount,
    0,
  );
  const totalRecommendations = analysisResults.reduce(
    (sum, r) => sum + r.stats.recommendationsCount,
    0,
  );
  ...
  for (const file of changedFiles) {
    try {
      ...
      if (issuesCount > 0 || warningsCount > 0) {
        console.log(
          `   ❌ Проблемы: ${issuesCount}, ⚠️  Предупреждения: ${warningsCount}, 💡 Рекомендации: ${recommendationsCount}`,
        );
      } else {
